...
//data exports we must provide for gpsd to be happy

/*
 * DATA:     android::Singleton<android::SensorManager>::sLock
 * USE:      INTERPOSE: a mutes that gpsd will insist on accessing
 ...
 *           the gpsd executable to be happy. It will lock nothnhing, but
 *           as long as it is a real lock and pthread_mutex_* funcs
 *           work on it, the gpsd executable will be happy.
 */
 ...
 *           there. In M this is entirely different, but the gpsd executable
 ...
 *           NULL (which is what it would be if the gpsd executable itself
 *           did the cleanup).
 */
 ...
 *           lib only services gpsd executable, it is easy for us to just do that
 *           and this provide the constructor that the gpsd executable wants.
 *           The package name we use if "gpsd.n8xx". Why not?
 */
void _ZN7android13SensorManagerC1Ev(void *sensorMgr)
{
    void *string;

    _ZN7android8String16C1EPKc(&string, "gpsd.n8xx");
...
{
    ALOGI("n80xx GPS interposition library loaded. Your GPS should work in M now.");
}
...
{
    ALOGI("n80xx GPS interposition library unloading. Goodbye...");
